 ...
 * <p>
 * Ported from the 1.20.1 branch. The public surface (categories and
 * {@link #addDefaultClass}) is identical, so addons can target both branches
 * with the same code. Default class lists are seeded to match this branch's
 * existing behaviour (vegetation only); the remaining categories start empty
 * and take effect as soon as classes are registered into them.
 * <p>
 * ONBOTTOM categories are accepted but have no effect on this branch yet,
 * since the onbottom offset engine has not been ported.
 */
...
public final class OffsetClasses {
    ...
    private static final List<Class<?>> MISC_ONTOP_CLASSES = new ArrayList<>();
    private static final List<Class<?>> FIRE_ONTOP_CLASSES = new ArrayList<>();
    private static final List<Class<?>> SNOWLAYER_ONTOP_CLASSES = new ArrayList<>();

    private static final List<Class<?>> VEGETATION_ONBOTTOM_CLASSES = new ArrayList<>();
    private static final List<Class<?>> MISC_ONBOTTOM_CLASSES = new ArrayList<>();

    private static final List<Class<?>> EXCLUDED_CLASSES = new ArrayList<>();
